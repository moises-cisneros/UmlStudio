...
export function buildCodegenApp(): Hono {
  ...
  app.get("/health", (c) => c.json({ status: "ok", service: "codegen", version: "0.1.0" }))
  ...
  app.post("/api/preview", async (c) => {
    ...
    if (!validation.valid) {
      return c.json(
        {
          error: "Model validation failed against canonical v4 schema",
    ...
    if (classCount === 0) {
      return c.json(
        {
          error:
            "Diagram contains no class nodes; at least 1 class is required to generate a backend",
        },
        422
      )
    }
  ...
  const handleOpenApiJson = async (c: import("hono").Context) => {
    ...
    const classCount = countClassNodes(body.model)
    if (classCount === 0) {
      return c.json({ error: "Diagram contains no class nodes; at least 1 class is required" }, 422)
    }
...
export function startCodegenServer(port: number = CODEGEN_DEFAULT_PORT): void {
  ...
  serve({ fetch: app.fetch, port }, (info) => {
    process.stdout.write(`codegen listening on http://${info.address}:${info.port}\n`)
  })
}
