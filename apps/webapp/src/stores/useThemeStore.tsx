...
if (typeof localStorage !== "undefined") {
  try {
    if (!localStorage.getItem(THEME_STORE_NAME) && localStorage.getItem(LEGACY_THEME_STORE_NAME)) {
      localStorage.setItem(THEME_STORE_NAME, localStorage.getItem(LEGACY_THEME_STORE_NAME)!)
...
const normalizePersistedThemeState = (state: PersistedThemeShape) => {
  ...
  return {
    ...
    userThemePreference,
    currentTheme,
  } satisfies Pick<ThemeState, "systemThemePreference" | "userThemePreference" | "currentTheme">
}
  ...
  persist(
    (set, get) => {
      const applyThemeState = (
        theme: ThemeMode,
        nextState: Partial<Pick<ThemeState, "systemThemePreference" | "userThemePreference">> = {}
      ) => {
      ...
      return {
        ...
        currentTheme: "light",
        setSystemThemePreference: (value: ThemeMode) => set({ systemThemePreference: value }),
        setUserThemePreference: (value: ThemeMode) => set({ userThemePreference: value }),
        ...
        toggleTheme: () => {
          ...
          const currentPreference = userThemePreference ?? systemThemePreference ?? "light"
          const newTheme: ThemeMode = currentPreference === "dark" ? "light" : "dark"
