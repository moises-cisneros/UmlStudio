...
import { getVersionRepository, subscribeToLocalVersionEvents } from "@/services/versionRepository"
...
export function ensureVersionStoreBootstrapped(queryClient: QueryClient = appQueryClient): void {
  if (dispose) return

  let prevModelIds = new Set(Object.keys(usePersistenceModelStore.getState().models))
  const unsubscribeModels = usePersistenceModelStore.subscribe((state, prev) => {
    ...
    for (const id of prevModelIds) {
      if (!next.has(id)) {
        getVersionRepository("local")
          .purgeDiagram?.(id)
          .catch((err: unknown) =>
            log.warn("purgeDiagram failed", err instanceof Error ? err.message : String(err))
          )
      }
    }
    prevModelIds = next
      ...
      .then(() => {
        ...
        if (versions && !versions.some((v) => v.id === previewing.versionId)) {
          queryClient.removeQueries({
            queryKey: versionKeys.body("local", msg.diagramId, previewing.versionId),
          })
          useVersionStore.getState().exitPreview()
        }
      })
      .catch((err: unknown) =>
        log.warn("Cross-tab refetch failed", err instanceof Error ? err.message : String(err))
      )
  })
