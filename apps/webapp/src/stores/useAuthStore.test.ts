...
describe("useAuthStore register (CU-12)", () => {
  it("auto-establishes the session with no second login call", async () => {
    const fetchMock = vi.fn(async () => jsonResponse(201, { token: "fresh-token", user: USER }))
    vi.stubGlobal("fetch", fetchMock)

    await useAuthStore.getState().register("Ada Modeler", "ada@example.com", "Modeler-2026!")
...
describe("useAuthStore updateProfile & changePassword (CU-14)", () => {
  ...
  it("sends password change request and handles errors", async () => {
    ...
    const fetchMock = vi.fn(async () => jsonResponse(400, { message: "Invalid current password" }))
    vi.stubGlobal("fetch", fetchMock)
