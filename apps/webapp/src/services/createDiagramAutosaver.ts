...
export function createDiagramAutosaver(opts: AutosaverOptions): DiagramAutosaver {
  ...
  const putOnce = async (model: UMLModel): Promise<boolean> => {
    ...
    for (;;) {
      try {
        const res = await DiagramApiClient.sendDiagramUpdate(opts.diagramId, model, { ifMatch })
