...
async function listSinglePage(
  ...
  const ascending = await db.getAllFromIndex("versions", "by_diagram_seq", diagramRange(diagramId))
...
export const LocalVersionRepository = {
  ...
  async create(diagramId, body, opts): Promise<CreateVersionResult> {
    ...
    const { row, evictedVersionIds, evictedKinds, totalAfter } = await commitVersion(db, {
      diagramId,
      body,
      meta: {
  ...
  async restore(diagramId, versionId, opts): Promise<RestoreVersionResult> {
    ...
    try {
      const tx = db.transaction(["versions", "versionBodies", "diagramMeta"], "readwrite")
      ...
      const label = target.description.trim() || target.name.trim() || `v${target.seq}`
      const currentUser = useAuthStore.getState().user
  ...
  async purgeDiagram(diagramId): Promise<void> {
    ...
    const ids = await versionsStore.index("by_diagram_seq").getAllKeys(diagramRange(diagramId))
    for (const id of ids) {
...
async function writeVersionInTx(
  ...
  await tx.objectStore("versionBodies").put({ diagramId, id: row.id, body: serializeBody(body) })
