...
export const isHomeDialogVariant = (props?: ModalProps | unknown) =>
  Boolean(props && typeof props === "object" && (props as ModalProps).dialogVariant === "home")
...
export const HomeDialogContent = ({
  ...
  <div data-testid={testId} className={cn("flex min-w-0 flex-col gap-5", className)}>
    {children}
  </div>
)
...
export const HomeDialogNotice = ({ children }: { children: ReactNode }) => (
  <Alert className="border-l-[5px] border-l-primary bg-accent-soft p-2">
    <AlertDescription className="text-sm font-normal text-foreground">{children}</AlertDescription>
  </Alert>
)
...
export const HomeDialogField = ({
  ...
  <Field className="gap-1.5">
    {htmlFor ? (
      <FieldLabel htmlFor={htmlFor} className="text-xs font-semibold text-foreground">
        {label}
      </FieldLabel>
    ) : (
...
export const HomeDialogTextInput = (props: InputHTMLAttributes<HTMLInputElement>) => (
  <Input
    {...props}
    className={cn(
...
export const HomeDialogOptionGroup = <T extends string>({
  ...
  return (
    <section className="flex flex-col gap-2" aria-labelledby={headingId}>
      <h3
        id={headingId}
        className={hideLabel ? "sr-only" : "text-xs font-semibold text-foreground"}
      >
        {label}
      </h3>
      <div
...
export const HomeDialogActions = ({
  ...
  <ModalFooterPortal>
    <DialogFooter>
      ...
      <Button variant="default" disabled={loading || confirmDisabled} onClick={onConfirm}>
