...
describe("uploadImageForVision", () => {
  it("returns the extracted model on 200", async () => {
    ...
    const fetchMock = vi.fn(async () =>
      jsonResponse(200, { model, confidence: { "n-user": 0.9 } }),
    );
  ...
  it("maps 413, 422, and 504 to typed errors with hints", async () => {
    for (const status of [413, 422, 504] as const) {
      const fetchMock = vi.fn(async () =>
        jsonResponse(status, { detail: "server says no" }),
      );
      const err = await uploadImageForVision(pngFile(), fetchMock).catch(
        (error: unknown) => error,
      );
      expect(err).toBeInstanceOf(VisionImportError);
...
describe("VisionImportDialog", () => {
  ...
  it("previews extracted classes then merges on confirm (FA-01)", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
        jsonResponse(200, { model, confidence: { "n-user": 0.82 } }),
      ),
    );
    ...
    fireEvent.change(
      screen.getByLabelText("Diagram photo", { selector: "input" }),
      {
