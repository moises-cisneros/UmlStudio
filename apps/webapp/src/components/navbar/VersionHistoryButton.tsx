  ...
  return (
    <Tooltip disabled={!iconOnly && isLg}>
      ...
      >
        ...
        <span className={iconOnly ? "hidden" : "hidden lg:inline"}>{t.menu.versionHistory}</span>
      </TooltipTrigger>
...
export const VersionHistoryButton = ({ variant }: VersionHistoryButtonProps) => {
  const diagramId = useDiagramIdFromPath()
  const { pathname } = useLocation()
  const hasVersioning =
    pathname.startsWith("/shared/") || pathname.startsWith("/local/") || Boolean(diagramId)
