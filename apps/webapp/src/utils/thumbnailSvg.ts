...
export const normalizeThumbnailSvg = (
  ...
  const viewBoxDimensions = getDimensionsFromViewBox(svgElement.getAttribute("viewBox"))

  const sourceWidth =
    widthAttribute ?? viewBoxDimensions?.width ?? fallbackWidth ?? THUMBNAIL_MAX_WIDTH
  const sourceHeight =
    heightAttribute ?? viewBoxDimensions?.height ?? fallbackHeight ?? THUMBNAIL_MAX_HEIGHT
...
export const renderThumbnailSvgFromModel = async (model: UMLModel): Promise<string> => {
  const { UmlStudioEditor } = await import("@umlstudio/core")
  ...
  let instance: InstanceType<typeof UmlStudioEditor> | undefined
  try {
    instance = new UmlStudioEditor(mountNode, { model })
    const exportedSvg = await instance.exportAsSVG({ svgMode: "compat" })
    return normalizeThumbnailSvg(exportedSvg.svg, exportedSvg.clip.width, exportedSvg.clip.height)
  } finally {
    instance?.destroy()
    mountNode.remove()
  }
