...
export const isDiagramView = (value: unknown): value is DiagramView =>
  typeof value === "string" && (Object.values(DiagramView) as string[]).includes(value)
...
export const getSharedDiagramViewOption = (view: unknown): SharedDiagramViewOption => {
  const normalizedView = normalizeSharedDiagramView(view)
  return (
    SHARED_DIAGRAM_VIEW_OPTIONS.find((option) => option.value === normalizedView) ??
...
): string => `${origin}/shared/${encodeURIComponent(diagramId)}?view=${encodeURIComponent(view)}`
