...
import { patchVersionInList, removeVersionFromList, replaceVersionInList } from "./versionListCache"
...
export function useEditVersionInfoMutation(kind: RepositoryKind, diagramId: string) {
  const queryClient = useQueryClient()
  const repo = getVersionRepository(kind)
...
export function useDeleteVersionMutation(kind: RepositoryKind, diagramId: string) {
  const queryClient = useQueryClient()
  const repo = getVersionRepository(kind)
  const listKey = versionKeys.list(kind, diagramId)
  return useMutation({
    mutationFn: ({ versionId }: { versionId: string }) => repo.delete(diagramId, versionId),
...
export function useRestoreVersionMutation(kind: RepositoryKind, diagramId: string) {
  const queryClient = useQueryClient()
  const repo = getVersionRepository(kind)
  return useMutation({
    ...
    onSuccess: ({ autoSnapshotVersionId }, { versionId }) => {
      ...
      useVersionStore.getState().completeRestore({
        ...
        restoredVersionName:
          ...
          restored?.name?.trim() ||
          (restored?.seq !== undefined ? `#${restored.seq}` : versionId.slice(0, 8)),
      })
...
export function useUndoRestoreMutation(kind: RepositoryKind) {
  ...
  const repo = getVersionRepository(kind)
  return useMutation({
    mutationFn: ({ diagramId, autoSnapshotVersionId, currentBody }: UndoRestoreVariables) =>
