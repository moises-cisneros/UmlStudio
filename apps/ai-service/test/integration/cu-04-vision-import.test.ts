...
function postVisionContract(
  ...
  const offendingIds = [
    ...model.nodes
      .filter((node) => !ALLOWED_NODE_TYPES.has(node.type))
      .map((node) => node.id),
    ...model.edges
      .filter((edge) => !ALLOWED_EDGE_TYPES.has(edge.type))
      .map((edge) => edge.id),
  ];
...
describe("INT-CU04: CU-04 vision image import (POST /api/vision)", () => {
  ...
  it("rejects payloads above 10MB with 413 and max-size guidance", () => {
    const result = postVisionContract(
      validFile({ size: VISION_MAX_BYTES + 1 }),
      fixture,
    );
  ...
  it("rejects images below 640x480 with 422", () => {
    const result = postVisionContract(
      validFile({ width: 320, height: 240 }),
      fixture,
    );
