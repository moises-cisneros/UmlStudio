...
export type Validated<S extends Schemas> = {
  [K in "body" | "query" | "params"]: S[K] extends ZodTypeAny ? z.infer<S[K]> : never
}
...
export function validate<S extends Schemas>(schemas: S, handler: TypedHandler<S>): Handler<AppEnv> {
  return async (c) => {
    ...
    for (const key of ["body", "query", "params"] as const) {
      ...
      if (!schema) continue
      const input =
        key === "body" ? await readJsonBody(c) : key === "query" ? c.req.query() : c.req.param()
