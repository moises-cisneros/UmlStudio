...
function computePoolMax(workerHeapMb: number): number {
  ...
  return Number.isFinite(override) && override >= 1 ? clamp(override, 1, 64) : derived
}
...
export class ConversionResource {
  ...
  private readonly workerStackMb = Number(process.env.CONVERTER_WORKER_STACK_MB ?? 8)
  private readonly poolMax = computePoolMax(this.workerMaxOldGenerationMb)
  private readonly poolMin = clamp(Number(process.env.CONVERTER_POOL_MIN ?? 1), 1, this.poolMax)
  private readonly maxQueueLength = clamp(
    Number(process.env.CONVERTER_MAX_QUEUE_LENGTH ?? this.poolMax * 8),
    8,
    512
  )
  private readonly maxQueueWaitMs = Number(process.env.CONVERTER_MAX_QUEUE_WAIT_MS ?? 10_000)
  private readonly recycleAfterRenders = Number(process.env.CONVERTER_RECYCLE_AFTER_RENDERS ?? 500)
  private readonly crashBackoffMaxMs = Number(process.env.CONVERTER_CRASH_BACKOFF_MAX_MS ?? 5_000)
  ...
  private spawn(): PooledWorker {
    ...
    pw.worker.on("error", (e) => this.onExit(pw, e instanceof Error ? e : new Error(String(e))))
  ...
  private onMessage(pw: PooledWorker, message: WorkerMessage) {
    ...
    if (message.ok) {
      job.resolve({
        mime: message.mime,
        data: typeof message.data === "string" ? message.data : Buffer.from(message.data),
      })
    } else if (message.code === "INVALID_MODEL_GEOMETRY") {
      job.reject(Errors.invalidParams(message.error.split("\n")[0] || message.error))
    } else {
  ...
  private onExit(pw: PooledWorker, error: Error) {
    ...
    const backoff = Math.min(250 * 2 ** (this.consecutiveCrashes - 1), this.crashBackoffMaxMs)
    const respawn = () => {
  ...
  private reapIdle() {
    ...
    for (const pw of [...this.workers]) {
      if (!pw.busy && this.workers.length > this.poolMin && now - pw.idleSince >= idleTtlMs) {
        this.workers = this.workers.filter((w) => w !== pw)
        void pw.worker.terminate().catch(() => undefined)
      }
    }
