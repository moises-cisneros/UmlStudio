...
export function mountCodegenRoutes(): Hono<AppEnv> {
  ...
  router.post("/codegen/spring-boot", async (c: Context<AppEnv>) => {
    ...
    const classNodes = (model.nodes ?? []).filter((n) => !n.type || n.type === "class")
    ...
    const artifactId = ((opts["artifactId"] as string) || model.title || "demo")
      .toLowerCase()
      .replace(/[^a-z0-9_-]/gi, "-")
    const cleanArt = artifactId.replace(/[^a-z0-9_]/gi, "")
    const packageName = (opts["packageName"] as string) || `${groupId}.${cleanArt}`
    const serverPort = (opts["serverPort"] as number) || 9000
    const dbHost = (opts["dbHost"] as string) || "localhost"
    const dbPort = (opts["dbPort"] as number) || 5433
    ...
    const inheritance = (opts["inheritance"] as SpringBootInheritanceStrategy) || "JOINED"
    ...
    // 5. Inject application.yml, springdoc-openapi dependency and 5 layers
    let rootPrefix = ""
    ...
    // Ensure springdoc-openapi dependency is present in pom.xml
    const pomPath = `${rootPrefix}pom.xml`
    const pomEntry = zip.file(pomPath)
    if (pomEntry) {
      const pomContent = await pomEntry.async("string")
      if (!pomContent.includes("springdoc-openapi-starter-webmvc-ui")) {
      ...
      <version>2.8.5</version>
        ...
        zip.file(pomPath, pomContent.replace("</dependencies>", springDocSnippet))
      }
    }
  ...
  const forwardToCodegen = async (
    ...
    const codegenUrl = process.env["CODEGEN_SERVICE_URL"] || "http://127.0.0.1:8002"
    try {
  ...
  router.post("/codegen/openapi/json", (c) => forwardToCodegen(c, "json", "application/json"))
  router.post("/codegen/openapi/yaml", (c) =>
    forwardToCodegen(c, "yaml", "text/yaml; charset=utf-8")
  )
  router.post("/codegen/openapi/postman", (c) => forwardToCodegen(c, "postman", "application/json"))
