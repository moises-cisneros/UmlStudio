...
import { type AuthService, InvalidPasswordError } from "../services/auth-service.js"
import { changePasswordSchema, updateProfileSchema } from "../auth/password-policy.js"
...
export function mountUserRoutes(deps: UserRouteDeps): Hono<AppEnv> {
  ...
  router.post("/change-password", async (c) => {
    ...
    if (!authUser) throw Errors.unauthorized()
    try {
      await auth.changePassword(authUser.id, parsed.data.currentPassword, parsed.data.newPassword)
