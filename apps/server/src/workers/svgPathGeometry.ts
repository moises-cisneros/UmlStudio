...
const distance = (a: PathPoint, b: PathPoint): number => Math.hypot(b.x - a.x, b.y - a.y)

/**
 * Flattens an SVG path `d` to an absolute polyline. Supports the commands
 * umlstudio emits (`M`/`L`/`H`/`V`/`Q`/`C`/`Z`, absolute or relative); an
    ...
    switch (cmd.toUpperCase()) {
      ...
      case "C": {
        ...
        for (let s = 1; s <= CUBIC_SAMPLES; s++) {
          ...
          const v = 1 - u
          pts.push({
            x: v * v * v * p0.x + 3 * v * v * u * c1.x + 3 * v * u * u * c2.x + u * u * u * e.x,
            y: v * v * v * p0.y + 3 * v * v * u * c1.y + 3 * v * u * u * c2.y + u * u * u * e.y,
          })
        }
        cur = e
        break
...
export function installSvgPathGeometry(win: Window): void {
  ...
  for (const name of ["SVGPathElement", "SVGGeometryElement", "SVGElement"]) {
    ...
    proto.getTotalLength = function () {
      return pathTotalLength((this as unknown as Element).getAttribute?.("d") ?? "")
    }
    proto.getPointAtLength = function (len: number) {
      return pathPointAtLength((this as unknown as Element).getAttribute?.("d") ?? "", len)
    }
  }
}
