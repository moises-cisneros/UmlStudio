...
function prepareForPng(svg: string, width: number, height: number): string {
  ...
  doc.querySelectorAll("text").forEach((text) => {
    ...
    text.querySelectorAll("tspan").forEach((tspan) => {
      ...
      if (tspanSize) tspan.setAttribute("stroke-width", String(STEM_DARKEN_EM * tspanSize))
    })
  })
...
async function render(
  ...
  if (format === "png") {
    ...
    const image = await loadImage(Buffer.from(prepareForPng(svg, width, height)))
...
parentPort?.on("message", async (message: WorkerRequest) => {
  ...
  try {
    const { mime, data } = await render(message.format, message.model, message.scale)
