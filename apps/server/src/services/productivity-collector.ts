...
export interface NodeContentionMetric {
  ...
  currentHolderUserId?: string | undefined
}
...
export class ProductivityCollector {
  ...
  public recordActivity(
    ...
    const collab = session.collaborators.get(user.userId)
    if (!collab) {
      const newCollab: CollaboratorSessionState = {
        userId: user.userId,
      ...
      session.collaborators.set(user.userId, newCollab)
    } else {
  ...
  public recordDisconnection(diagramId: string, userId: string, timestamp?: number): void {
  ...
  public getReport(diagramId: string, now: number = Date.now()): DiagramProductivityReport {
    const session = this.getOrCreateDiagramSession(diagramId)
    ...
    bottlenecks.sort(
      (a, b) =>
        b.contentionCount - a.contentionCount || b.totalLockDurationMs - a.totalLockDurationMs
    )

    // Calculate velocity (per hour of active session time)
    const effectiveHours = Math.max(totalActiveSec / 3600, 1 / 60)
    const classesPerHour = Math.round((session.totalClassesCreated / effectiveHours) * 10) / 10
    const methodsPerHour = Math.round((session.totalMethodsCreated / effectiveHours) * 10) / 10
    const refactorsPerHour = Math.round((session.totalRefactors / effectiveHours) * 10) / 10
