...
describe("INT-CU08: Case of Use CU-08 Versioning, Snapshots and Restoration Integration", () => {
  ...
  app.route("/api", mountDiagramRoutes({ config, redis: {} as Redis }, relayMock))
  ...
  function createTestApp(r: Redis, auth?: AuthService) {
    ...
    testApp.route(
      "/api",
      mountDiagramRoutes({ config, redis: r, auth: auth ?? authService }, relayMock)
    )
    testApp.route(
      "/api",
      mountVersionRoutes({ config, redis: r, auth: auth ?? authService }, relayMock)
    )
    return testApp
  }
  ...
  it("executes full lifecycle: manual snapshot, reverse-chronological list, non-destructive preview, atomic restore and undo", async () => {
    ...
    const v1Res = await testApp.request(`http://localhost/api/diagrams/${diagramId}/versions`, {
      method: "POST",
    ...
    const existsVersionBody = await redis.exists(k.versionBody(diagramId, v1Data.id))
    const existsVersionMeta = await redis.exists(k.versionMeta(diagramId, v1Data.id))
    ...
    const v2Res = await testApp.request(`http://localhost/api/diagrams/${diagramId}/versions`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  it("binds immutable snapshot authorship to verified authenticated user identity (CU-08 / CU-11)", async () => {
    ...
    const commitRes = await testApp.request(`http://localhost/api/diagrams/${diagramId}/versions`, {
      method: "POST",
      headers: {
