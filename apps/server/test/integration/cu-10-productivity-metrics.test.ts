...
import { createHeadlessSync, MessageType, type YjsSync } from "@umlstudio/core/internals"
...
function encodeFrame(type: MessageType, payload: Uint8Array = new Uint8Array(0)): string {
...
function connectPeer(port: number, diagramId: string, options: ConnectOptions = {}): VirtualPeer {
...
describe("INT-CU10: Analizar Productividad, Cuellos de Botella y Gestión del Tiempo", () => {
  ...
  it("RF-40 & RF-41: debe registrar telemetría activa y detectar colisiones de contención de locks entre colaboradores", async () => {
    ...
    const aliceCollab = report.collaborators.find((c) => c.userId === "user-alice")
    const bobCollab = report.collaborators.find((c) => c.userId === "user-bob")
  ...
  it("RF-40: debe deducir correctamente lapsos de inactividad superiores a 60 segundos", () => {
    ...
    const t1 = t0 + 100_000
    productivityCollector.recordActivity("diag-idle-test", { userId: "u1" }, { timestamp: t1 })
  ...
  it("RF-42: debe calcular velocidad de diseño y cadencia de refactorización", () => {
    ...
    const report = productivityCollector.getReport("diag-velocity-test", t0 + 3600_000)
  ...
  it("RF-43: debe exponer endpoints REST para consulta y telemetría de productividad", async () => {
    ...
    const postRes = await app.request(`/api/diagrams/${diagramId}/productivity/telemetry`, {
      method: "POST",
    ...
    const getRes = await app.request(`/api/diagrams/${diagramId}/productivity`)
