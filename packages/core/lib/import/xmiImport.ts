...
import { DiagramNodeTypeRecord, DiagramEdgeTypeRecord, type DiagramEdgeType } from "../modelElementTypes";
 ...
 */
export function importXmiDiagram(
  xmiString: string,
  options: XmiImportOptions = {},
): UMLModel {
  ...
  if (!sanitizedXmi.includes("xmlns:xmi=")) {
    sanitizedXmi = sanitizedXmi.replace(/<xmi:XMI/i, '<xmi:XMI xmlns:xmi="http://schema.omg.org/spec/XMI/2.1"');
  }
  if (!sanitizedXmi.includes("xmlns:uml=")) {
    sanitizedXmi = sanitizedXmi.replace(/<xmi:XMI/i, '<xmi:XMI xmlns:uml="http://schema.omg.org/spec/UML/2.1"');
  }
  ...
  const modelEl = doc.querySelector("Model, uml\\:Model") || doc.getElementsByTagName("uml:Model")[0];
  const modelName =
    options.defaultTitle ||
    modelEl?.getAttribute("name") ||
    "Imported XMI Diagram";
  ...
  allPackagedElements.forEach((el) => {
    ...
    const validClassTypes = ["class", "interface", "enumeration", "package"];
    ...
    const attrElements = Array.from(el.querySelectorAll(":scope > ownedAttribute, :scope > property"));
    attrElements.forEach((attrEl, idx) => {
      ...
      const attrId = attrEl.getAttribute("xmi:id") || attrEl.getAttribute("id") || `attr-${internalId}-${idx}`;
      attributes.push({ id: attrId, name: formatted });
    });

    // Extract Operations
    const methods: Array<{ id: string; name: string }> = [];
    const opElements = Array.from(el.querySelectorAll(":scope > ownedOperation, :scope > operation"));
    opElements.forEach((opEl, idx) => {
      ...
      // Parameters
      const paramElements = Array.from(opEl.querySelectorAll(":scope > ownedParameter, :scope > parameter"));
      ...
      const opId = opEl.getAttribute("xmi:id") || opEl.getAttribute("id") || `op-${internalId}-${idx}`;
  ...
  allPackagedElements.forEach((el) => {
    ...
    genElements.forEach((genEl, idx) => {
      const targetXmiId = genEl.getAttribute("general") || genEl.getAttribute("href")?.split("#")?.[1];
      ...
      if (targetNodeId) {
        ...
        const edgeId = genEl.getAttribute("xmi:id") || `edge-gen-${sourceNodeId}-${targetNodeId}-${idx}`;
        edges.push({
          id: edgeId,
    ...
    const realElements = Array.from(el.querySelectorAll(":scope > interfaceRealization, interfaceRealization"));
    realElements.forEach((realEl, idx) => {
      ...
      if (targetNodeId) {
        ...
        const targetNode = nodeMap.get(targetNodeId);
        const { sourceHandle, targetHandle } = deriveOptimalHandles(sourceNode, targetNode);
        const edgeId = realEl.getAttribute("xmi:id") || `edge-real-${sourceNodeId}-${targetNodeId}-${idx}`;
        edges.push({
          id: edgeId,
  ...
  connectorElements.forEach((connEl) => {
    ...
    const existing = edges.find(
      (e) => (edgeId && e.id === edgeId) || (e.source === sourceNodeId && e.target === targetNodeId && e.type === edgeType),
    );
    ...
    if (isAssoc) {
      const rawSrcMult = sourceEl?.querySelector("type")?.getAttribute("multiplicity") || labelsEl?.getAttribute("lb");
      const rawTgtMult = targetEl?.querySelector("type")?.getAttribute("multiplicity") || labelsEl?.getAttribute("rb");
      srcMult = sanitizeMultiplicity(rawSrcMult);
      tgtMult = sanitizeMultiplicity(rawTgtMult);

      const rawSourceRole = sourceEl?.querySelector("role")?.getAttribute("name") || labelsEl?.getAttribute("lt");
      const rawTargetRole = targetEl?.querySelector("role")?.getAttribute("name") || labelsEl?.getAttribute("rt");
