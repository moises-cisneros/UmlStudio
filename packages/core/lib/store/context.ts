...
export const DiagramStoreContext = createContext<StoreApi<DiagramStore> | null>(null)

export const MetadataStoreContext = createContext<StoreApi<MetadataStore> | null>(null)
...
export const AlignmentGuidesStoreContext = createContext<StoreApi<AlignmentGuidesStore> | null>(
  null
)

export const EdgeGeometryStoreContext = createContext<StoreApi<EdgeGeometryStore> | null>(null)

export const OverlayStoreContext = createContext<StoreApi<OverlayStore> | null>(null)
...
export const useAlignmentGuidesStore = <T>(selector: (state: AlignmentGuidesStore) => T): T => {
  const store = use(AlignmentGuidesStoreContext)
  if (!store) throw new Error("AlignmentGuidesStoreContext not provided")
  return useStore(store, selector)
}

export const useEdgeGeometryStore = <T>(selector: (state: EdgeGeometryStore) => T): T => {
