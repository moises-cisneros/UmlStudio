...
import { getNodesMap, getEdgesMap, getAssessments, reconcileYMap, STORE_ORIGIN } from "@/sync/ydoc"
...
function stripComputedSegmentsFromEdge(edge: Edge): Edge {
  if (!edge.data || !Object.prototype.hasOwnProperty.call(edge.data, "computedSegments")) {
    return edge
  }
...
export type DiagramStore = {
  ...
  lastPlacedElementId: string | null
  associationClassPrompt: { fromNodeId: string | null; error?: boolean } | null
  setAssociationClassPrompt: (prompt: { fromNodeId: string | null; error?: boolean } | null) => void
...
export const createDiagramStore = (ydoc: Y.Doc): UseBoundStore<StoreApi<DiagramStore>> =>
  create<DiagramStore>()(
    devtools(
      subscribeWithSelector((set, get) => {
        ...
        let draggingNodesPublisher: ((draggingNodes: DraggingNode[] | null) => void) | null = null
        ...
        return {
          ...
          setAssociationClassPrompt: (prompt) => {
            set({ associationClassPrompt: prompt }, undefined, "setAssociationClassPrompt")
          },
          ...
          setCollaborationEnabled: (enabled) => {
            ...
            set({ collaborationEnabled: enabled }, undefined, "setCollaborationEnabled")
          },
          ...
          setSelectedElementsId: (payload) => {
            const selectedElementIds =
              typeof payload === "function" ? payload(get().selectedElementIds) : payload
          ...
          setLastPlacedElementId: (id) => {
            set({ lastPlacedElementId: id }, undefined, "setLastPlacedElementId")
          },

          toggleInteractiveElement: (elementId) => {
            const isNode = get().nodes.some((node) => node.id === elementId)
            const isNestedNodeElement = getNestedNodeElementIds(get().nodes).has(elementId)
            ...
            set(
              (state) => ({
                ...
                interactiveRelationships: isEdge
                  ? toggleInteractiveRecord(state.interactiveRelationships, elementId)
          ...
          isElementInteractive: (elementId) => {
            return !!(
              get().interactiveElements[elementId] || get().interactiveRelationships[elementId]
            )
          },
          ...
          addEdge: (edge) => {
            ...
            set({ edges: [...get().edges, persistedEdge] }, undefined, "addEdge")
          },
          setNodes: (payload) => {
            const nodes = typeof payload === "function" ? payload(get().nodes) : payload
          ...
          setEdges: (payload) => {
            const edges = typeof payload === "function" ? payload(get().edges) : payload
          ...
          onNodesChange: (changes) => {
            ...
            if (selectChanges.length > 0) {
              selectChanges.forEach((change) => {
                if (change.selected) {
                  set(
                    (state) => ({
                      selectedElementIds: [...state.selectedElementIds, change.id],
                    }),
                    undefined,
                  ...
                  set(
                    (state) => ({
                      selectedElementIds: state.selectedElementIds.filter((id) => id !== change.id),
                    }),
                    undefined,
            ...
            const gestureInFlight = nextNodes.some((n) => n.dragging || n.resizing)

            let publishedLiveFrames = false
            if (get().collaborationEnabled && !get().previewMode && draggingNodesPublisher) {
              ...
              for (const change of filteredChanges) {
                if (change.type === "position" && change.dragging === true) {
                  ...
                  const id = change.type === "replace" ? change.item.id : change.id
            ...
            transactStore(() => {
              for (const change of filteredChanges) {
                if (change.type === "add" || change.type === "replace") {
                  ...
                  getNodesMap(ydoc).set(change.item.id, stripSelected(change.item))
              ...
              if (resizeSettled) {
                reconcileYMap(getNodesMap(ydoc), nodeEntriesForPersistence(nextNodes))
              }
            })
            const hasRemovals = filteredChanges.some((c) => c.type === "remove")
            const currentInteractiveElements = get().interactiveElements
            const currentInteractiveRelationships = get().interactiveRelationships
            ...
            if (hasRemovals) {
              ...
              nextInteractiveRelationships = prunedInteractive?.relationships ?? {}
            }
          ...
          onEdgesChange: (changes) => {
            ...
            const changesWithoutSelect = changes.filter((change) => change.type !== "select")
            ...
            transactStore(() => {
              for (const change of changes) {
                if (change.type === "add" || change.type === "replace") {
                  const persistedEdge = stripComputedSegmentsFromEdge(change.item)
                  getEdgesMap(ydoc).set(persistedEdge.id, persistedEdge)
                } else if (change.type === "remove") {
                  set(
                    (state) => ({
          ...
          updateNodesFromYjs: () => {
            ...
            const removedNodes = get().nodes.filter(
              (node) => !preserveSelectedNodesAfterYdoc.some((n) => n.id === node.id)
            )
            if (removedNodes.length > 0) {
              set(
                (state) => ({
                  selectedElementIds: state.selectedElementIds.filter(
                    (id) => !removedNodes.some((removedNode) => removedNode.id === id)
                  ),
                }),
                undefined,
          ...
          updateEdgesFromYjs: () => {
            const preserveSelectedEdgesAfterYdoc = Array.from(getEdgesMap(ydoc).values()).map(
              (edge) => {
            ...
            const removedEdges = get().edges.filter(
              (edge) => !preserveSelectedEdgesAfterYdoc.some((e) => e.id === edge.id)
            )
            if (removedEdges.length > 0) {
              set(
                (state) => ({
                  selectedElementIds: state.selectedElementIds.filter(
                    (id) => !removedEdges.some((removedEdge) => removedEdge.id === id)
                  ),
                }),
                undefined,
          ...
          setAssessments: (payload) => {
            const assessments = typeof payload === "function" ? payload(get().assessments) : payload
