...
export const getPositionOnCanvas = (node: Node, allNodes: ReadonlyArray<Node>): XYPosition => {
  ...
  const visit = (node: Node) => {
    ...
    if (node.parentId) {
      ...
      if (parentNode) {
        ...
        log.warn(`Parent node with id ${node.parentId} not found for node ${node.id}`)
      }
    }
