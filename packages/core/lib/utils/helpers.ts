...
export const getAssessmentNameForArtemis = (
  ...
  if (foundNode) {
    return {
      name: (foundNode.data?.name as string) || foundNode.type || "Unnamed Node",
      type: foundNode.type,
    }
  }
  ...
  for (const node of model.nodes) {
    if (node.data) {
      if ("attributes" in node.data && Array.isArray(node.data.attributes)) {
        const foundAttribute = node.data.attributes.find((attr) => attr.id === elementId)
        if (foundAttribute) {
          return {
      ...
      if ("methods" in node.data && Array.isArray(node.data.methods)) {
        const foundMethod = node.data.methods.find((method) => method.id === elementId)
        if (foundMethod) {
          return {
      ...
      if ("actionRows" in node.data && Array.isArray(node.data.actionRows)) {
        const foundActionRow = node.data.actionRows.find((actionRow) => actionRow.id === elementId)
        if (foundActionRow) {
          return {
...
export const getNodeAssessmentDataByNodeElementId = (
  ...
  for (const node of model.nodes) {
    if (node.data) {
      if ("attributes" in node.data && Array.isArray(node.data.attributes)) {
        const foundAttribute = node.data.attributes.find((attr) => attr.id === nodeElementId)
        if (foundAttribute) {
          return {
            elementId: nodeElementId,
            elementType: nodeAssessment.elementType,
      ...
      if ("methods" in node.data && Array.isArray(node.data.methods)) {
        const foundMethod = node.data.methods.find((method) => method.id === nodeElementId)
        if (foundMethod) {
          return {
            elementId: nodeElementId,
            elementType: nodeAssessment.elementType,
