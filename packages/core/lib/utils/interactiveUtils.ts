...
type InteractivePruneNode = Pick<UmlStudioNode, "id"> &
  Partial<Pick<UmlStudioNode, "data">>;
...
function filterInteractiveRecord(
  ...
  return Object.fromEntries(
    entries.filter(([id, included]) => included && allowedIds.has(id)),
  );
}
...
export function pruneInteractiveElements(
  ...
  if (
    ...
    relationships === interactive.relationships
  ) {
    return interactive;
  }
