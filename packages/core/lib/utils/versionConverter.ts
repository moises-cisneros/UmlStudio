...
function normalizeImportedInterfaceGeometry(
  ...
  const isInterfaceNode = nodeType === "componentInterface" || nodeType === "deploymentInterface"
...
export function convertV3EdgeTypeToV4(v3Type: string, flowType?: string): string {
...
function convertV3RelationshipToV4Edge(relationship: V3UMLRelationship): UmlStudioEdge {
  const edgeType = convertV3EdgeTypeToV4(relationship.type, relationship.flowType)
  let points: IPoint[] = []
...
export function convertV3ToV4(v3Data: V3DiagramFormat | V3UMLModel): UMLModel {
  const model: V3UMLModel = (v3Data as V3DiagramFormat).model || (v3Data as V3UMLModel)
  ...
  const edges: UmlStudioEdge[] = Object.values(model.relationships).map((relationship) =>
      ...
      (Object.values(model.interactive.elements ?? {}).some(Boolean) ||
        ...
        ? {
            elements: Object.fromEntries(
              Object.entries(model.interactive.elements ?? {}).filter(([, included]) => included)
            ),
...
export function normalizeStraightEdgeWaypoints(model: UMLModel): UMLModel {
  ...
  if (minor < STRAIGHT_WAYPOINT_MODEL_MINOR) {
    for (const edge of model.edges) {
      ...
      const data = edge.data as (Record<string, unknown> & { points?: unknown }) | null | undefined
