      ...
      style={{
        ...
        ...(sideRail
          ? {
              ...
              maxWidth: "100%",
              overflow:
                control.id === "umlstudio:palette" ? "auto" : "visible",
            }
          : null),
        ...control.style,
      }}
  ...
  const flushMeasure = useCallback(() => {
    ...
    for (const [id, el] of elByIdRef.current) {
      ...
      const edge = BAND_REGIONS.includes(control.region)
        ? 0
        : parseFloat(
            getComputedStyle(el).getPropertyValue("--umlstudio-chrome-edge"),
          ) || 0;
    ...
    if (grid) {
      ...
      const clear = (region: OverlayRegion) => {
        ...
        const edge =
          parseFloat(styles.getPropertyValue("--umlstudio-chrome-edge")) || 0;
        return Math.ceil(height + edge);
      };
      grid.style.setProperty(
        "--umlstudio-left-rail-top-clearance",
        `${clear("top-left")}px`,
      );
      grid.style.setProperty(
        "--umlstudio-left-rail-bottom-clearance",
        `${clear("bottom-left")}px`,
      );
      grid.style.setProperty(
        "--umlstudio-right-rail-top-clearance",
        `${clear("top-right")}px`,
      );
      grid.style.setProperty(
            ...
            <div
              ...
              style={{
                ...
                ...(region === "left-rail"
                  ? {
                      paddingTop:
                        "var(--umlstudio-left-rail-top-clearance, 0px)",
                      paddingBottom:
                        "var(--umlstudio-left-rail-bottom-clearance, 0px)",
                    }
                  : null),
                ...(region === "right-rail"
                  ? {
                      paddingTop:
                        "var(--umlstudio-right-rail-top-clearance, 0px)",
                      paddingBottom:
                        "var(--umlstudio-right-rail-bottom-clearance, 0px)",
                    }
                  : null),
              }}
            >
