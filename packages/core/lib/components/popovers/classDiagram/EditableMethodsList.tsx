  ...
  return (
    ...
    >
      ...
      >
        ...
        <NodeStyleEditor
          nodeData={item}
          handleDataFieldUpdate={(key, value) => onMethodChange(item.id, key, value)}
          sideElements={[
            <TagPicker
              key={`tags_${item.id}`}
              tags={item.tags ?? []}
...
export const EditableMethodsList: React.FC<Props> = ({ nodeId }) => {
  ...
  const patchMethods = (updatedMethods: typeof methods) => {
    setNodes((nodes) =>
      nodes.map((node) =>
        node.id === nodeId ? { ...node, data: { ...node.data, methods: updatedMethods } } : node
      )
    )
  }

  const handleMethodChange = (id: string, key: string, value: string) => {
    patchMethods(methods.map((item) => (item.id === id ? { ...item, [key]: value } : item)))
  }

  const handleTagsChange = (id: string, tags: string[]) => {
    patchMethods(methods.map((item) => (item.id === id ? withTags(item, tags) : item)))
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <Code2 width={14} height={14} style={{ color: "var(--brand-cyan, #00d8ff)" }} />
      ...
      <div className="umlstudio-add-row">
        ...
        />
        <IconButton ariaLabel={t.addMethod} tooltip={t.addMethod} onClick={handleAddItem}>
