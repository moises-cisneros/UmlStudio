...
const KIND_TO_DATA: Record<ClassKind, { stereotype?: ClassStereotype; isAbstract: boolean }> = {
...
export const ClassEditPopover: React.FC<PopoverProps> = ({ elementId }) => {
  ...
  const handleTagsUpdate = (tags: string[]) => {
    setNodes((nodes) =>
      nodes.map((node) =>
        node.id === elementId ? { ...node, data: withTags(node.data, tags) } : node
      )
    )
  }
  ...
  const setKind = (next: ClassKind) => {
    ...
    const keywordLinePx =
      LAYOUT.DEFAULT_HEADER_HEIGHT_WITH_STEREOTYPE - LAYOUT.DEFAULT_HEADER_HEIGHT
    const delta = ((hasKeyword(next) ? 1 : 0) - (hasKeyword(currentKind) ? 1 : 0)) * keywordLinePx
    setNodes((nodes) =>
      nodes.map((node) => {
