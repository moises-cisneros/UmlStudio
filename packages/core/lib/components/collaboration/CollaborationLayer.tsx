...
const getElementTargets = (container: HTMLElement, elementId: string) => {
  ...
  return [
    container.querySelector<HTMLElement>(`.react-flow__node[data-id="${escapedId}"]`),
    container.querySelector<HTMLElement>(`.react-flow__edge[data-id="${escapedId}"]`),
...
function CollaboratorSelectionHighlights({
  ...
  const [remoteHighlights, setRemoteHighlights] = useState<Map<string, string>>(() => new Map())
  ...
  useEffect(() => {
    ...
    for (const [elementId, color] of remoteHighlights.entries()) {
      ...
      for (const target of targets) {
        target.style.setProperty("--umlstudio-collaboration-highlight-color", color)
  ...
  }, [active, diagramId, edges, highlightSignature, nodes, previewMode, remoteHighlights])

  return null
}

const sameViewport = (a: CollaborationViewport | null, b: CollaborationViewport | null) =>
...
function ViewportFollow({
  ...
  const pendingViewportRef = useRef<CollaborationViewport | null>(null)
  const broadcastRafRef = useRef(0)
  const applyingRemoteRef = useRef(false)
  const lastAppliedRef = useRef<CollaborationViewport | null>(null)

  const flushViewport = useCallback(() => {
    broadcastRafRef.current = 0
    if (pendingViewportRef.current) {
      awareness.setLocalAwarenessViewport(pendingViewportRef.current)
  ...
  const handleViewportChange = useCallback(
    (viewport: CollaborationViewport) => {
      if (applyingRemoteRef.current || sameViewport(viewport, lastAppliedRef.current)) return
      if (followedClientId !== null) onStopFollowing()
      pendingViewportRef.current = viewport
      if (!broadcastRafRef.current) {
        broadcastRafRef.current = window.requestAnimationFrame(flushViewport)
      }
    },
  ...
  useEffect(() => {
    ...
    const applyTargetViewport = (states: Map<number, CollaborationState>) => {
      ...
      if (!viewport || sameViewport(viewport, lastAppliedRef.current)) return

      lastAppliedRef.current = viewport
    ...
    const unsubscribe = awareness.subscribeToAwarenessChanges(applyTargetViewport)
...
function FollowIndicator({
  ...
  return (
    <>
      ...
      />
      <div className="umlstudio-collaboration-follow-banner" style={{ borderColor: target.color }}>
        <span
          aria-hidden="true"
          className="umlstudio-collaboration-follow-banner-dot"
          style={{ backgroundColor: target.color }}
        />
        <span className="umlstudio-collaboration-follow-banner-text" role="status">
...
export function CollaborationLayer({ options, awareness }: CollaborationLayerProps) {
  ...
  return (
    <>
      ...
      {followActive && followTarget && (
        <FollowIndicator target={followTarget} onStopFollowing={stopFollowing} />
      )}
    </>
  )
}
