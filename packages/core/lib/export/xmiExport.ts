...
import { DiagramEdgeTypeRecord, DiagramNodeTypeRecord } from "../modelElementTypes";
...
function mapVisibilityToXmi(symbol?: string): "public" | "private" | "protected" | "package" {
  switch (symbol) {
    case "-":
      return "private";
    case "#":
...
function mapPrimitiveTypeHref(typeName: string): string {
  ...
  if (norm === "string") return "http://schema.omg.org/spec/UML/2.1/uml.xml#String";
  if (norm === "integer" || norm === "int") return "http://schema.omg.org/spec/UML/2.1/uml.xml#Integer";
  if (norm === "real" || norm === "float" || norm === "double" || norm === "number") {
    return "http://schema.omg.org/spec/UML/2.1/uml.xml#Real";
  }
  if (norm === "boolean" || norm === "bool") return "http://schema.omg.org/spec/UML/2.1/uml.xml#Boolean";
...
function buildMultiplicityXml(
  ...
  if (trimmed === "1") {
    lines.push(`          <lowerValue xmi:type="uml:LiteralInteger" xmi:id="${idPrefix}_lv" value="1" />`);
    lines.push(`          <upperValue xmi:type="uml:LiteralInteger" xmi:id="${idPrefix}_uv" value="1" />`);
  } else if (trimmed === "0..1") {
    lines.push(`          <lowerValue xmi:type="uml:LiteralInteger" xmi:id="${idPrefix}_lv" value="0" />`);
    lines.push(`          <upperValue xmi:type="uml:LiteralInteger" xmi:id="${idPrefix}_uv" value="1" />`);
  } else if (trimmed === "1..*") {
    lines.push(`          <lowerValue xmi:type="uml:LiteralInteger" xmi:id="${idPrefix}_lv" value="1" />`);
    lines.push(`          <upperValue xmi:type="uml:LiteralUnlimitedNatural" xmi:id="${idPrefix}_uv" value="-1" />`);
    ...
    if (parts.length === 2) {
      lines.push(`          <lowerValue xmi:type="uml:LiteralInteger" xmi:id="${idPrefix}_lv" value="${parts[0].trim()}" />`);
      if (parts[1].trim() === "*") {
        lines.push(`          <upperValue xmi:type="uml:LiteralUnlimitedNatural" xmi:id="${idPrefix}_uv" value="-1" />`);
      } else {
        lines.push(`          <upperValue xmi:type="uml:LiteralInteger" xmi:id="${idPrefix}_uv" value="${parts[1].trim()}" />`);
      }
    }
  }
  return lines;
...
function serializeNode(node: UmlStudioNode, allEdges: UmlStudioEdge[]): string[] {
  const lines: string[] = [];
  ...
  if (isPackage) elementType = "uml:Package";
  ...
  lines.push(`    <packagedElement xmi:type="${elementType}" xmi:id="${escapeXml(node.id)}" name="${name}"${abstractAttr}>`);

  // Serialise Attributes
  if (Array.isArray(nodeData?.attributes)) {
    nodeData.attributes.forEach((attr: { id?: string; name: string }, idx: number) => {
      const raw = attr.name || "";
      const match = raw.match(/^\s*([+\-#~])?\s*([a-zA-Z0-9_$]+)(?:\s*:\s*([a-zA-Z0-9_$<>,. ]+))?/);
      ...
      lines.push(`      <ownedAttribute xmi:type="uml:Property" xmi:id="${attrId}" name="${attrName}" visibility="${visibility}">`);
      if (typeName) {
        lines.push(`        <type xmi:type="uml:PrimitiveType" href="${escapeXml(mapPrimitiveTypeHref(typeName))}" />`);
      }
  ...
  if (Array.isArray(nodeData?.methods)) {
    nodeData.methods.forEach((method: { id?: string; name: string }, idx: number) => {
      const raw = method.name || "";
      const match = raw.match(/^\s*([+\-#~])?\s*([a-zA-Z0-9_$]+)\s*(?:\((.*?)\))?(?:\s*:\s*([a-zA-Z0-9_$<>,. ]+))?/);
      const visSymbol = match?.[1] || "+";
      ...
      lines.push(`      <ownedOperation xmi:type="uml:Operation" xmi:id="${opId}" name="${methodName}" visibility="${visibility}">`);

      // In parameters
      if (paramsRaw && paramsRaw.trim()) {
        const paramTokens = paramsRaw.split(",");
        paramTokens.forEach((pt, pIdx) => {
          const pMatch = pt.trim().match(/^([a-zA-Z0-9_$]+)(?:\s*:\s*([a-zA-Z0-9_$<>,. ]+))?/);
          const pName = escapeXml(pMatch?.[1] || `param_${pIdx}`);
          const pType = pMatch?.[2]?.trim();
          const pId = `${opId}_p_${pIdx}`;
          lines.push(`        <ownedParameter xmi:type="uml:Parameter" xmi:id="${pId}" name="${pName}" direction="in">`);
          if (pType) {
            lines.push(`          <type xmi:type="uml:PrimitiveType" href="${escapeXml(mapPrimitiveTypeHref(pType))}" />`);
          }
      ...
      if (returnType && returnType.toLowerCase() !== "void") {
        ...
        lines.push(`          <type xmi:type="uml:PrimitiveType" href="${escapeXml(mapPrimitiveTypeHref(returnType))}" />`);
        lines.push("        </ownedParameter>");
      } else {
        lines.push(`        <ownedParameter xmi:type="uml:Parameter" xmi:id="${retId}" name="return" direction="return" type="EAnone_void" />`);
      }
  ...
  nodeEdges.forEach((edge) => {
    if (edge.type === DiagramEdgeTypeRecord.ClassInheritance) {
      lines.push(`      <generalization xmi:type="uml:Generalization" xmi:id="${escapeXml(edge.id)}" general="${escapeXml(edge.target)}" />`);
...
function serializeModelEdges(edges: UmlStudioEdge[]): string[] {
  ...
  edges.forEach((edge) => {
    ...
    const rawLabel = typeof edgeData?.label === "string" ? edgeData.label.trim() : "";
    const edgeName = rawLabel && !rawLabel.startsWith("assoc_") ? escapeXml(rawLabel) : "";
    const srcPropId = `prop_${edge.id}_src`;
    const tgtPropId = `prop_${edge.id}_tgt`;

    lines.push(`    <packagedElement xmi:type="uml:Association" xmi:id="${escapeXml(edge.id)}" name="${edgeName}">`);
    ...
    lines.push(`      <ownedEnd xmi:type="uml:Property" xmi:id="${srcPropId}" name="${srcRoleName}" type="${escapeXml(edge.source)}" association="${escapeXml(edge.id)}">`);
    const srcMultLines = buildMultiplicityXml(edgeData?.sourceMultiplicity, `${srcPropId}_mult`, false);
    ...
    const aggAttr = isComposition ? ' aggregation="composite"' : isAggregation ? ' aggregation="shared"' : "";
    lines.push(`      <ownedEnd xmi:type="uml:Property" xmi:id="${tgtPropId}" name="${tgtRoleName}" type="${escapeXml(edge.target)}"${aggAttr} association="${escapeXml(edge.id)}">`);
    const tgtMultLines = buildMultiplicityXml(edgeData?.targetMultiplicity, `${tgtPropId}_mult`, false);
...
function serializeEnterpriseArchitectConnectors(
  ...
  edges.forEach((edge) => {
    ...
    const srcMult = typeof edgeData?.sourceMultiplicity === "string" && edgeData.sourceMultiplicity.trim()
      ? escapeXml(edgeData.sourceMultiplicity.trim())
      : "";
    const tgtMult = typeof edgeData?.targetMultiplicity === "string" && edgeData.targetMultiplicity.trim()
    ...
    lines.push(`          <model type="Class" name="${sourceName}"/>`);
    if (sourceRole) {
      lines.push(`          <role name="${sourceRole}" visibility="Public" targetScope="instance"/>`);
    }
    const srcMultAttr = srcMult ? ` multiplicity="${srcMult}"` : "";
    lines.push(`          <type${srcMultAttr} aggregation="${sourceAgg}" containment="Unspecified"/>`);
    ...
    lines.push(`          <model type="Class" name="${targetName}"/>`);
    if (targetRole) {
      lines.push(`          <role name="${targetRole}" visibility="Public" targetScope="instance"/>`);
    }
    const tgtMultAttr = tgtMult ? ` multiplicity="${tgtMult}"` : "";
    lines.push(`          <type${tgtMultAttr} aggregation="${targetAgg}" containment="Unspecified"/>`);
    lines.push('          <constraints/>');
    lines.push('          <modifiers isOrdered="false" changeable="none" isNavigable="false"/>');
    lines.push('          <style value="Union=0;Derived=0;AllowDuplicates=0;Owned=0;Navigable=Unspecified;"/>');
    lines.push('          <documentation/>');
    ...
    if (lbAttr || ltAttr || mtAttr || rbAttr || rtAttr) {
      lines.push(`        <labels${lbAttr}${ltAttr}${mtAttr}${rbAttr}${rtAttr}/>`);
...
function serializeEnterpriseArchitectExtension(
  ...
  const cleanId = (modelId || "0001").replace(/[^a-zA-Z0-9_]/g, "_").toUpperCase();
  const now = new Date();
  ...
  nodes.forEach((node, idx) => {
    ...
    const dataObj = typeof node.data === "object" && node.data !== null ? (node.data as Record<string, unknown>) : undefined;
    const sizeObj = dataObj && typeof dataObj.size === "object" && dataObj.size !== null ? (dataObj.size as Record<string, unknown>) : undefined;
    ...
    ptInstancesList.push(`SX=${left};SY=${top};EX=${right};EY=${bottom};SND=${seqno};GUID={${guid}};`);
  ...
  nodes.forEach((node) => {
    const dataObj = typeof node.data === "object" && node.data !== null ? (node.data as Record<string, unknown>) : undefined;
    const nodeName = escapeXml(typeof dataObj?.name === "string" ? dataObj.name : node.id);
    lines.push(`      <element xmi:idref="${escapeXml(node.id)}" xmi:type="uml:Class" name="${nodeName}" scope="public">`);
    lines.push(`        <model package="${packageId}" tpos="0" ea_eleType="element"/>`);
    lines.push('        <properties isSpecification="false" sType="Class" nType="0" scope="public" isRoot="false" isLeaf="false" isAbstract="false" isActive="false"/>');
    lines.push(`        <project author="UmlStudio" version="1.0" created="${nowStr}" modified="${nowStr}" status="Proposed"/>`);
    lines.push("      </element>");
  });
  ...
  lines.push('        <packagedElement xmi:type="uml:Package" xmi:id="EAnoneTypesPackage" name="EA_none_Types_Package" visibility="public">');
  lines.push('          <packagedElement xmi:type="uml:PrimitiveType" xmi:id="EAnone_void" name="void" visibility="public"/>');
  lines.push("        </packagedElement>");
  lines.push("      </packagedElement>");
  lines.push("    </primitivetypes>");
  ...
  lines.push(`        <model package="${packageId}" localID="2" owner="${packageId}"/>`);
  lines.push(`        <properties name="${escapeXml(diagramName)}" type="Logical"/>`);
  lines.push(`        <project author="UmlStudio" version="1.0" created="${nowStr}" modified="${nowStr}"/>`);
  lines.push('        <style appearance="BackColor=-1;BorderColor=-1;BorderWidth=-1;FontColor=-1;VSwimLanes=1;HSwimLanes=1;BorderStyle=0;"/>');
  lines.push(`        <extendedProperties ptInstances="${escapeXml(ptInstancesStr)}"/>`);
  lines.push("        <elements>");
...
export async function exportToXmi(
  ...
  const cleanId = (model.id || "0001").replace(/[^a-zA-Z0-9_]/g, "_").toUpperCase();
  ...
  const eaExtensionLines = serializeEnterpriseArchitectExtension(modelId, packageId, name, nodes, edges);
