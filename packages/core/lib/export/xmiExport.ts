...
function serializeNode(node: UmlStudioNode, allEdges: UmlStudioEdge[]): string[] {
  ...
  const isAssocClass = Boolean(
    ...
    allEdges.some(
      (e) => (e.data as Record<string, unknown> | undefined)?.associationClassNodeId === node.id
    )
  )
  ...
  if (Array.isArray(nodeData?.attributes)) {
    nodeData.attributes.forEach((attr: { id?: string; name: string }, idx: number) => {
      ...
      const attrName = escapeXml(parsed?.name || raw.trim() || `attr_${idx}`)
  ...
  if (Array.isArray(nodeData?.methods)) {
    nodeData.methods.forEach((method: { id?: string; name: string }, idx: number) => {
      ...
      const visSymbol = parsed?.visibilitySymbol || "+"
      const methodName = escapeXml(parsed?.name || raw.trim() || `op_${idx}`)
  ...
  if (isAssocClass) {
    const assocEdge = allEdges.find(
      (e) =>
        (e.data as Record<string, unknown> | undefined)?.associationClassNodeId === node.id ||
...
function serializeEnterpriseArchitectConnectors(
  ...
  edges.forEach((edge) => {
    ...
    const srcMult =
      typeof edgeData?.sourceMultiplicity === "string" && edgeData.sourceMultiplicity.trim()
        ? escapeXml(edgeData.sourceMultiplicity.trim())
        : ""
    const tgtMult =
      typeof edgeData?.targetMultiplicity === "string" && edgeData.targetMultiplicity.trim()
  ...
  nodes.forEach((node) => {
    ...
    if (isAssocClass) {
      const assocEdge = edges.find(
        (e) =>
          (e.data as Record<string, unknown> | undefined)?.associationClassNodeId === node.id ||
          (dataObj as Record<string, unknown> | undefined)?.associationEdgeId === e.id
      )
...
export async function exportToXmi(
  ...
  rawNodes.forEach((node) => {
    ...
    const nodeName = String((node.data as { name?: string })?.name || "").trim()
    if (!nodeName) {
  ...
  const footer = ["    </packagedElement>", "  </uml:Model>", ...eaExtensionLines, "</xmi:XMI>"]
