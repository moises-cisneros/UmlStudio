...
describe("INT-CU05: Case of Use CU-05 XMI Import/Export Interoperability", () => {
  ...
  it("Step 2: exports UMLModel to Enterprise Architect compliant XMI 2.1 preserving taxonomy", async () => {
    ...
    expect(xmiContent).toMatch(
      /<extendedProperties ptInstances="[^"]*SX=\d+;SY=\d+;EX=\d+;EY=\d+;SND=\d+;GUID=\{[^}]+\};/
    )
  })
  ...
  it("Step 4: exports empty name in connectors and associations when unlabelled and no assoc_ in labels", async () => {
    ...
    expect(xmiContent).not.toMatch(
      /<packagedElement[^>]*xmi:type="uml:Association"[^>]*name="assoc_[^"]+"/
    )
  ...
  it("Step 6: imports native Enterprise Architect associations from Example.xmi with type xmi:idref and connectors", () => {
    ...
    // Verify nodes (including Class G AssociationClass)
    expect(model.nodes.length).toBe(8)
    ...
    expect(names).toContain("Class G")

    // Verify associations were successfully parsed from Example.xmi (6 regular + 1 association class)
    ...
    const assocClassEdge = model.edges.find((e) => e.data?.associationClassNodeId)
    expect(assocClassEdge).toBeDefined()
  ...
  it("Step 8: ensures non-associations never export/import roles or multiplicities, and associations without them remain empty", async () => {
    ...
    const cleanAssocMatch = xmi.match(
      /<connector xmi:idref="edge-clean-assoc"[\s\S]*?<\/connector>/
    )
    ...
    const confAssocMatch = xmi.match(
      /<connector xmi:idref="edge-configured-assoc"[\s\S]*?<\/connector>/
    )
    ...
    expect(confAssocMatch![0]).toContain(
      '<labels lb="1" lt="+owner" mt="manages" rb="0..*" rt="+asset"/>'
    )
