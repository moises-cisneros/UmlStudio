...
describe("convertV3NodeTypeToV4", () => {
  ...
  it("maps CommunicationObject to communicationObjectName", () => {
    expect(convertV3NodeTypeToV4("CommunicationObject")).toBe("communicationObjectName")
  })
...
describe("convertV3EdgeTypeToV4", () => {
  it.each([
    ...
    ["ComponentInterfaceRequiredQuarter", undefined, "ComponentRequiredQuarterInterface"],
    ["ComponentInterfaceRequiredThreeQuarter", undefined, "ComponentRequiredThreeQuarterInterface"],
    ...
    ["DeploymentInterfaceRequiredQuarter", undefined, "DeploymentRequiredQuarterInterface"],
    [
  ...
  ] as [string, undefined, string][])("maps %s to %s", (input, flowType, expected) => {
  ...
  describe("BPMNFlow with flowType", () => {
    it("maps BPMNFlow + sequence to BPMNSequenceFlow", () => {
      expect(convertV3EdgeTypeToV4("BPMNFlow", "sequence")).toBe("BPMNSequenceFlow")
    })

    it("maps BPMNFlow + message to BPMNMessageFlow", () => {
      expect(convertV3EdgeTypeToV4("BPMNFlow", "message")).toBe("BPMNMessageFlow")
    })

    it("maps BPMNFlow + association to BPMNAssociationFlow", () => {
      expect(convertV3EdgeTypeToV4("BPMNFlow", "association")).toBe("BPMNAssociationFlow")
    })

    it("maps BPMNFlow + dataAssociation to BPMNDataAssociationFlow", () => {
      expect(convertV3EdgeTypeToV4("BPMNFlow", "dataAssociation")).toBe("BPMNDataAssociationFlow")
    })

    it("defaults BPMNFlow with unknown flowType to BPMNSequenceFlow", () => {
      expect(convertV3EdgeTypeToV4("BPMNFlow", "unknownFlowType")).toBe("BPMNSequenceFlow")
    })
...
describe("convertV3MessagesToV4", () => {
  ...
  it("returns the array as-is when already in V4 format", () => {
    const v4Messages = [{ id: "m1", text: "hello", direction: "source" as const }]
...
describe("convertV3ToV4", () => {
  ...
  it("converts CommunicationObject with child attributes/methods", () => {
    ...
    expect((result.nodes[0].data.attributes as Record<string, unknown>[])[0].name).toBe("x")
  })
...
describe("isV3Format", () => {
  ...
  it("returns false when model version is not a string", () => {
    expect(isV3Format({ model: { version: 3, elements: {}, relationships: {} } })).toBe(false)
  })
  ...
  it("returns false when flat model has array elements (V2-like)", () => {
    expect(isV3Format({ version: "3.0.0", elements: [], relationships: [] })).toBe(true)
  })
...
describe("isV4Format", () => {
  ...
  it("rejects V4 edges with non-object data", () => {
    expect(isV4Format(makeV4Model({ edges: [{ id: "bad-data", data: "invalid" }] }))).toBe(false)
  })
...
describe("importDiagram", () => {
  ...
  it("throws for completely unsupported format", () => {
    expect(() => importDiagram({ foo: "bar" })).toThrow("Unsupported diagram format")
  })
...
describe("normalizeClassStereotypes (legacy 4.x class nodes)", () => {
  ...
  it("lowercases the Interface / Enumeration keywords to the UML spelling", () => {
    ...
    normalizeClassStereotypes(model)
    expect((model.nodes[0].data as { stereotype: string }).stereotype).toBe("interface")
    expect((model.nodes[1].data as { stereotype: string }).stereotype).toBe("enumeration")
  })
  ...
  it("runs on the editor load path via importDiagram", () => {
    const result = importDiagram(makeV4ClassModel([makeV4ClassNode("Abstract", 110)]))
...
describe("normalizeStraightEdgeWaypoints", () => {
  ...
  it("clears cached full routes from straight edges saved before 4.2", () => {
    ...
    expect((model.edges[1].data as { points: unknown[] }).points).toEqual(points)
  })
  ...
  it("preserves authored interior waypoints from 4.2 onward", () => {
    ...
    normalizeStraightEdgeWaypoints(model)
    expect((model.edges[0].data as { points: unknown[] }).points).toEqual(points)
  })
...
describe("legacy non-UML node sanitization", () => {
  ...
  it("drops a v3 TitleAndDescription element without a titleAndDesctiption alias", () => {
    ...
    expect(result.nodes).toHaveLength(0)
    expect(result.nodes.map((n) => n.type)).not.toContain("titleAndDesctiption")
  })

  it("does not alias TitleAndDescription in convertV3NodeTypeToV4", () => {
    expect(convertV3NodeTypeToV4("TitleAndDescription")).not.toBe("titleAndDesctiption")
  })
